...
const OthersProfile = ({ params }) => {
            ...
            <div className="flex gap-3 flex-col">
              ...
              <div className="flex gap-5 uppercase">
                ...
                <div className=" border-b-2 border-blue-400 ">
                  <h1>
                    {userInfo.pass_out_year < new Date().getFullYear() &&
                      "Graduated"}
                    {userInfo.pass_out_year === new Date().getFullYear() &&
                      "4 Year"}
                    {userInfo.pass_out_year - new Date().getFullYear() === 1 &&
                      "3 Year"}
                    {userInfo.pass_out_year - new Date().getFullYear() === 2 &&
                      "2 Year"}
                    {userInfo.pass_out_year - new Date().getFullYear() === 3 &&
                      "1 Year"}
                  </h1>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <h2 className="text-md font-bold text-blue-100 ">
                  Connections
                </h2>
  ...
  className={`text-white font-bold w-full text-xl rounded-xl ${
    ...
    connectionStatus === 'removeRequest' ? 'bg-red-500' :
    connectionStatus === 'acceptRequest' ? 'bg-blue-500' :
    connectionStatus === 'sendRequest' ? 'bg-purple-500' :
    ''
  }`}
  onClick={() => {
